...
class dataHandler(): # Class pour la gestion des datas
    ...
    def create_message(self, UtilisateurId: int,msgUtilisateur: str, msgLunaris : str): #Permet de créer un objet "message" qui aurat qui correspondra aux message de luna et ceux de l'utilisateur
        cursor = self.con.cursor() # Permet la communication avec la base de donnée
        query = f"INSERT INTO message (utilisateurId, msgUtilisateur, msgLunaris) VALUES (?, ?, ?);" # Permet d'inserer dans la table message "message", le message de l'utilisateur et celui du bot !
        cursor.execute(query, (UtilisateurId, msgUtilisateur, msgLunaris,)) # Pour executer les insertions des messages dans la table message !
